"""
Created on Sat Mar 16 14:59:09 2019
...
class model():
    def __init__(self):
        ...
        self.image_encodings = pickle.load(open("Image_encoding/image_encodings.p", "rb"))
        self.captions = None
        self.img_id = None
        self.values()
        
    def values(self):
        dataframe2 = pd.read_csv('data/Text/trainimgs.txt', delimiter=',', header=None)
        dataframe = dataframe2.loc[:len(dataframe2)//2, :]
        self.captions = []
        self.img_id = []
        self.no_samples=0
    ...
    def data_process(self, batch_size): #permet de bien entrainer le modèle LSTM
        partial_captions = []
        next_words = []
        images = []
        total_count = 0
    ...
    def create_model(self, ret_model = False): #Notre modèle
