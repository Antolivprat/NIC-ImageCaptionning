"""
Created on Sat Mar 16 15:00:09 2019

@author: Antoine Prat
"""

import model_text as desc
from keras.preprocessing import sequence
import numpy as np

def process_caption(sd, caption):
    tokens = caption.split()
    #On coupe au premier '<end>'
    if '<end>' not in tokens: #S'il n'y a pas de '<end>' alors on en rajoute un à la fin
        tokens.append('<end>')
    tokens = tokens[1:]
    terminate_id = tokens.index('<end>') #On coupe
    tokens = tokens[:terminate_id]
    return " ".join([word for word in tokens]) #On rejoint et on renvoie
...
def generate_captions(sd, model, encoded_images, beam_size):  #tm.generate_captions(sd, model, encoded_images, beam_size=3)
    ...
    while len(capt_seq[0][0]) < max_cap_length: #Tant qu'on ne dépasse pas la probabilité max
        ...
        capt_seq = capt_seq[-beam_size:] #On regarde les beam_size premières
